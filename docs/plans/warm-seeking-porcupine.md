...
Проект `your-claude-engineer` — автономная оболочка AI агента на Claude Agent SDK. Сейчас это плоская структура из 20+ Python файлов в корне без `pyproject.toml`. Агент работает последовательно: один оркестратор делегирует субагентам по очереди.
