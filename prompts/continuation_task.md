...
- Read `.task_project.json` for project IDs (including meta_issue_id)

### Step 2: Get Status from Task Server (CHECK FOR COMPLETION)
Delegate to `task` agent:
"Read .task_project.json, then:
...
3. Compare done count to total_issues from .task_project.json
...
1. Ask task agent to add "PROJECT COMPLETE" comment to META issue
2. Ask telegram agent to send ":tada: Project complete!" notification
...
"Send message: :white_check_mark: Completed: [issue title]"

### Step 8: Session Handoff (if ending session)
If ending the session, delegate to `task` agent:
